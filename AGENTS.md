...
- **A scheme is a real, physically-enacted sequence, not an instant effect.** Once validated, `CultSystem.beginCultSchemePreparation` seeds a synthetic `activeBlocks` occupation for the leader directly (bypassing `ScheduleSystem.startBlock`, whose `'work'` case would otherwise fire ordinary work side-effects at the leader's current position before they've gone anywhere) and starts them moving toward their job building — `CultScheme.status` walks through `'proposed' → 'traveling' → 'preparing' → 'resolved'/'rejected'`, mirroring how `gatherCultForSummoning`/`advanceSummoningProcess` already manage a leader's block in place across many ticks. Occupying `activeBlocks` is what blocks `ScheduleSystem.startDueScheduleBlocks()` from interrupting the leader with a fresh decision mid-scheme; once arrived (`<=2` tiles of the building center), the leader holds a flat 30-minute `'preparing'` activity (`CultSystem.SCHEME_PREP_DURATION_MINUTES`) before `executeCultScheme` actually fires.
- **Interruption is free.** Any existing priority flow that already clears or replaces an agent's `activeBlocks` entry (an attack, exile, a court summons) is detected on the next `advanceCultSchemes` tick via the `ActiveBlockEntry.schemeId` tag no longer matching, and the scheme is abandoned (`status: 'rejected'`) — no changes needed to those other systems.
- **Witnessing seeds ordinary evidence rather than a scripted "caught" outcome.** While `'preparing'`, `CultSystem.maybeWitnessSchemePreparation` checks for a nearby Priest, Town Guard, or Inquisitor (not a fellow cultist) via the existing `Agent.getNearbyAgents` proximity helper; on a personality-weighted roll (witness curiosity raises it, leader caution lowers it), it seeds one ordinary, cult-flavored rumour naming the leader via the existing `createRumour`/`registerAgentCreatedRumour` — feeding the same investigation/court pipeline documented in SOCIAL.md rather than a new discovery mechanic. The scheme itself is not slowed, weakened, or aborted by being witnessed; only a new rumour now exists.
