...
* **Cult Schemes** — Once per simulated day, a cult leader may devise a covert scheme that uses their own trade as cover: a farmer's tainted grain, a carpenter's hidden idol, a merchant's marked trinket, a priest's consecrated relic. An LLM only ever chooses *what kind* of tactic (plant a relic, or quietly sway nearby villagers) and *how bold* a posture to take — the engine alone determines actual potency from the leader's ambition, faith, cult size, and reputation, capped by that risk choice. A planted object becomes an ordinary Forbidden Relic, so it's discovered and reacted to through the same mechanic as any other relic; a validator rejects any scheme a leader's vocation doesn't plausibly afford, with a job-flavored fallback scheme if the LLM fails twice.
