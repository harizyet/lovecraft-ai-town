...
const KEY_MOMENT_LABELS: Record<StoryMomentKind, string> = {
  ...
  cult_leader_corrupted: "a cult leader's secret and terrible corruption -- in the hushed, quietly dreadful style of a coastal town's ruin, à la H. P. Lovecraft's Innsmouth -- an ordinary villager, trusted in whatever mundane trade they still practice by day, privately descending into something ancient and hungry, unsuspected by the neighbours and even the very followers who kneel to them in secret",
  ...
  demon_created: 'the first ritual summoning of a bound entity',
  ...
  deity_relic_created: "not a mortal's confession but a deity's own hand reaching down to leave something behind -- a relic manifested directly onto the earth by divine will, its words never filtered through a frightened human author; write this as an act of the numinous intruding physically on the ordinary village, an object that did not exist a moment ago and now simply is, waiting for whichever unlucky soul finds it first",
  alderman_named: "a village assembly voting, unanimously and without a single dissent, to grant one of its own binding authority over every future court verdict and assembly question -- and the unspoken irony that the one now entrusted with that office secretly leads a cult the villagers who just elevated him know nothing about; write this as a quiet civic ceremony curdling into dramatic irony, the crowd's trust misplaced in a way only the chronicle's reader can see",
  knight_called: "a Knight arriving from beyond the village's borders, drawn by word of a death toll grown too high to ignore -- write this as the moment an outside authority, armored and grim, rides into a village that has already lost too many of its own, a stranger's blade arriving to answer a violence the villagers could not contain themselves",
...
Write up to ${remainingTurns} more turns continuing this conversation, alternating strictly starting with ${respondentName}. Speak as medieval villagers would: plain, period-appropriate speech fitting their trade and station, with no modern slang, technology, or institutions. Stop earlier than ${remainingTurns} turns if the conversation naturally winds down. Return ONLY JSON: {"turns":[{"speaker":"exact name","dialogue":"..."}]}.`
  ...
  private formatConversationPersona(agent: Agent, other: Agent, allAgents: Agent[]): string {
    ...
    return `${state.name} (${state.currentJob ?? 'no job'}): personality - aggression ${state.personality.aggression.toFixed(1)}, friendliness ${state.personality.friendliness.toFixed(1)}, curiosity ${state.personality.curiosity.toFixed(1)}, caution ${state.personality.caution.toFixed(1)}. Emotional state: ${state.emotionalState}. Relationship to ${other.state.name}: ${relationship ? `${relationship.type} (${relationship.strength}/100)` : 'not personally known'}.${recent.length ? ` Recent memory: ${recent.map((e) => e.description).join('; ')}` : ''}${state.secretProphet ? ` ${state.name} is secretly the true leader of ${state.cult?.name ?? 'a hidden cult'} but must speak here as an ordinary ${state.currentJob ?? 'villager'} and never reveal this.` : ''}${agent.isInsane() ? ` ${state.name} is insane${state.permanentInsanity ? ` (permanently, from ${state.permanentInsanity.reason})` : ' (from severe low sanity)'}: their lines should be unstable, panicked, obsessive, fearful, or erratic -- fractured thoughts, non sequiturs, paranoia, or raving -- never calm, coherent, ordinary small talk.` : this.formatExistentialPersona(state)}${this.formatDreamPersona(state)}`
  }
...
${state.secretProphet ? `SECRET PROPHET: Schedule a normal day as ${state.currentJob ? `a ${state.currentJob}` : 'an ordinary villager'} -- your usual duties, routines, and appearances -- to keep your cover intact. Weave your true responsibilities to ${state.cult?.name ?? 'your hidden congregation'} (preach, ritual, invite_cult, build_shrine) in only where they can plausibly pass as ordinary ${state.currentJob ?? 'daily'} activity; never schedule anything that would announce your true allegiance to the village.` : ''}
...
${state.secretProphet ? `- SECRET PROPHET: You outwardly remain ${state.currentJob ? `a ${state.currentJob}` : 'an ordinary villager'}, trusted by the whole town, but in truth you now lead ${state.cult?.name ?? 'a hidden congregation'} in secret. Keep performing your ordinary duties as cover -- your usual work, routines, an appearance of unremarkable life -- and never use investigate, interrogate, or call_inquisitor against your own true cult or its members. Your public words may sound ordinary while carrying a private meaning only your true believers grasp.` : ''}
